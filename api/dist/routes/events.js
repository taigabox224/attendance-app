...
export async function registerEventRoutes(app) {
    ...
    app.get('/api/events', { preHandler: requireAuth }, async (req) => {
        ...
        if (userRole === 'sysadmin') {
           ...
           ORDER BY e.start_at DESC`)
            ...
            rows = db
                .prepare(`SELECT * FROM events WHERE published = 1 ORDER BY start_at DESC`)
                .all();
        }
        const attendeeStmt = db.prepare(`SELECT status FROM event_attendees WHERE event_id = ? AND user_id = ?`);
        const receptionistStmt = db.prepare(`SELECT 1 FROM event_receptionists WHERE event_id = ? AND user_id = ?`);
        const events = rows.map((r) => {
            const att = attendeeStmt.get(r.id, userId);
    ...
    app.get('/api/events/:id', { preHandler: requireAuth }, async (req, reply) => {
        ...
        const attendees = db
            .prepare(`SELECT a.id, a.user_id, a.is_observer, a.observer_name,
        ...
        const receptionists = db
            .prepare(`SELECT er.user_id, u.name FROM event_receptionists er
    ...
    // 作成 (editor 以上)
    app.post('/api/events', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        db.prepare(`INSERT INTO events (
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, d.title, d.start_at, d.end_at ?? null, d.response_deadline ?? null, d.committee ?? null, d.location ?? null, d.description ?? null, req.user.sub, d.published ? 1 : 0, d.has_afterparty ? 1 : 0, d.afterparty_title ?? null, d.afterparty_location ?? null, d.afterparty_description ?? null, now, now);
        const row = db
            .prepare(`SELECT * FROM events WHERE id = ?`)
            .get(id);
    ...
    // 更新 (editor 以上、部分更新)
    app.patch('/api/events/:id', { preHandler: requireRole('editor') }, async (req, reply) => {
    ...
    // 削除 (editor 以上)
    app.delete('/api/events/:id', { preHandler: requireRole('editor') }, async (req, reply) => {
        const result = db
    ...
    app.post('/api/events/:id/attendees', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        const addUser = db.prepare(`INSERT OR IGNORE INTO event_attendees
        ...
        const addObserver = db.prepare(`INSERT INTO event_attendees
           (id, event_id, user_id, is_observer, observer_name, status, after_status, created_at, updated_at)
         VALUES (?, ?, NULL, 1, ?, 'yes', ?, ?, ?)`);
        let added = 0;
        const txn = db.transaction(() => {
            for (const uid of user_ids) {
                const r = addUser.run(generateAttendeeId(), req.params.id, uid, memberAfter, now, now);
                if (r.changes === 1)
                    added++;
            }
            for (const obs of observers) {
                const r = addObserver.run(generateAttendeeId(), req.params.id, obs.name, observerAfter, now, now);
                if (r.changes === 1)
                    added++;
            }
        });
    ...
    // 参加者を削除 (editor 以上)
    app.delete('/api/events/:id/attendees/:attendee_id', { preHandler: requireRole('editor') }, async (req, reply) => {
        const result = db
    ...
    app.patch('/api/events/:id/attendees/:attendee_id', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        const existing = db
            ...
            .get(req.params.attendee_id, req.params.id);
        if (!existing)
            return reply.code(404).send({ error: 'Attendee not found' });
        const sets = [];
        const params = [];
        ...
        if (d.checked_in_at !== undefined) {
            const v = d.checked_in_at === 'now' ? new Date().toISOString() : d.checked_in_at;
        ...
        db.prepare(`UPDATE event_attendees SET ${sets.join(', ')} WHERE id = ?`).run(...params);
        return { ok: true };
    });
    ...
    app.patch('/api/events/:id/rsvp', { preHandler: requireAuth }, async (req, reply) => {
        ...
        const now = new Date().toISOString();
        db.prepare(`UPDATE event_attendees
         SET status = ?, after_status = ?, updated_at = ?
         WHERE id = ?`).run(status, after_status ?? null, now, myRow.id);
        return { ok: true };
    });
    // 受付用 QR トークン発行 (要認証 + 自分が attendee であること)。
    // 中身は JWT。/checkin で検証する。
    app.get('/api/events/:id/qr-token', { preHandler: requireAuth }, async (req, reply) => {
        const myRow = db
            .prepare(`SELECT id FROM event_attendees WHERE event_id = ? AND user_id = ?`)
            .get(req.params.id, req.user.sub);
        if (!myRow) {
    ...
    app.put('/api/events/:id/receptionists', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        const apply = db.transaction(() => {
            db.prepare(`DELETE FROM event_receptionists WHERE event_id = ?`).run(req.params.id);
            const insert = db.prepare(`INSERT OR IGNORE INTO event_receptionists (event_id, user_id, created_at)
           VALUES (?, ?, ?)`);
    ...
    app.post('/api/events/:id/checkin', { preHandler: requireAuth }, async (req, reply) => {
        const userRole = req.user.role;
        if (userRole === 'editor') {
            return reply
                .code(403)
        ...
        if (userRole !== 'sysadmin') {
            const row = db
                .prepare(`SELECT 1 FROM event_receptionists WHERE event_id = ? AND user_id = ?`)
                .get(req.params.id, req.user.sub);
            if (!row) {
                return reply
                    .code(403)
