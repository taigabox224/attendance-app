...
const USER_STATUSES = ['active', 'inactive', 'left'];
...
export async function registerAdminRoutes(app) {
    app.post('/api/admin/users', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        db.prepare(`INSERT INTO users (
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`).run(id, email, normalized, name, family_name, given_name, passwordHash, role, department ?? null, title ?? null, now, // email_verified_at: 管理者経由なので認証済み扱い
        now, now);
    ...
    app.get('/api/admin/users', { preHandler: requireRole('editor') }, async () => {
    ...
    app.patch('/api/admin/users/:id', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        if (data.family_name !== undefined || data.given_name !== undefined) {
            const currentNames = db
                .prepare(`SELECT family_name, given_name FROM users WHERE id = ?`)
                .get(req.params.id);
    ...
    app.delete('/api/admin/users/:id', { preHandler: requireRole('editor') }, async (req, reply) => {
    ...
    app.put('/api/admin/users/order', { preHandler: requireRole('editor') }, async (req, reply) => {
        ...
        const apply = db.transaction(() => {
            // 一旦全員 NULL に戻す
            db.prepare(`UPDATE users SET display_order = NULL, updated_at = ?`).run(now);
            // 指定 ID に 1..N をセット
            const set = db.prepare(`UPDATE users SET display_order = ?, updated_at = ? WHERE id = ?`);
