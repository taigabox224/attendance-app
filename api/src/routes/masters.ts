...
function readMaster(kind: Kind): string[] {
  const row = db
    .prepare(`SELECT values_json FROM master_lists WHERE kind = ?`)
    .get(kind) as { values_json: string } | undefined;
  if (!row) return [];
  try {
...
export async function registerMasterRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put<{ Params: { kind: string } }>(
    ...
    async (req, reply) => {
      ...
      db.prepare(
        `INSERT INTO master_lists (kind, values_json, updated_at)
         VALUES (?, ?, ?)
         ON CONFLICT(kind) DO UPDATE SET values_json = excluded.values_json,
                                          updated_at = excluded.updated_at`,
      ).run(kind, JSON.stringify(deduped), now);
