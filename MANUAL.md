...
- [4. 一般ユーザー向け操作](#4-一般ユーザー向け操作)
  ...
  - [4-4. プロフィールを編集する](#4-4-プロフィールを編集する)
  - [4-5. パスワードを変更する](#4-5-パスワードを変更する)
...
- [6. 管理者向け操作 (編集者・システム管理者)](#6-管理者向け操作)
  ...
  - [6-2. イベントを新規作成する](#6-2-イベントを新規作成する)
  - [6-3. イベントを編集する](#6-3-イベントを編集する)
  - [6-4. 出席内訳と CSV 書き出し](#6-4-出席内訳と-csv-書き出し)
  - [6-5. ユーザー管理 (ユーザー登録 / 編集)](#6-5-ユーザー管理)
  - [6-6. 設定 (委員会・役職マスター / 表示順 / 参加者リスト)](#6-6-設定)
- [7. よくある質問](#7-よくある質問)
- [8. 困った時は](#8-困った時は)
...
編集者・システム管理者(=管理者ロール)だけが行える操作の解説です。
**全ての操作で管理者モードに切替える必要があるわけではありません** — モードと機能の対応関係は次の通り:

| 機能 | 必要なモード | 補足 |
|---|---|---|
| 6-2. **イベント新規作成** | **ユーザーモード** | 「+ 新規作成」 ボタンはユーザーモード時のみ表示 |
| 6-3. イベント編集 / 削除 | どちらでも可 | イベント詳細 → 歯車メニューから |
| 6-4. 出席内訳 / CSV書出し | 管理者モード | イベント詳細画面が admin ビューに切替わる |
| 6-5. ユーザー管理 | 管理者モード | 上部「ユーザー」 タブから |
| 6-6. 設定 (マスター/表示順/参加者リスト) | 管理者モード | ユーザー管理画面の歯車から |

「管理者モード」 は一覧確認・集計・ユーザー管理のためのモード。**イベントを作る時はユーザーモードのまま** が運用ルールです。
...
> ⚠️ **「+ 新規作成」 ボタンはユーザーモードでのみ表示されます**。管理者モード中のイベント一覧 (「登録されているイベント」 画面) には出ません。ユーザーモードに切替えてから進めてください。

<img src="docs/screenshots/10-event-create-top.png" width="320" alt="イベント作成 上部" />

1. ユーザーモードで /events を開く
2. イベント一覧画面の右上「**+ 新規作成**」ボタンをタップ
3. 入力フォーム:
   - **タイトル** (必須)
   - **開始日 / 時刻** (必須)、**終了日 / 時刻** (任意)
...
> 編集者は user モードでも歯車から「イベントを編集」 にアクセスできます (会場での修正対応)。
